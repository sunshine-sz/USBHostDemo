...
public class OkHttpClientManager {
     ...
     * @param callback
     */
    private void _getAsynWithHeader(String url, final StringCallback callback) {
        Request.Builder builder = new Request.Builder();
        deliveryResult(callback, builder.url(url).build());
    }
     ...
     * @param jsonObject
     */
    private void _postJson(String url, final StringCallback callback, String jsonObject) {
        Request request = buildPostRequest(url, jsonObject);
        deliveryResult(callback, request);
    }

    private void _postJsonAndHeader(String url, final StringCallback callback, JSONObject jsonObject) {
//        Request request = buildPostRequest(url, jsonObject);
        Request request = buildPostRequestAndHeader(url, jsonObject);
        deliveryResult(callback, request);
    }
...
//    }
     ...
     * @return request
     */
    private Request buildPostRequest(String url, String jsonObject) {
        Log.i("OkHttp3","发送:"+jsonObject);
        RequestBody requestBody = RequestBody.create(JSON, jsonObject);
        Request.Builder builder = new Request.Builder();
        return builder.url(url).post(requestBody).build();
    }

    private Request buildPostRequestAndHeader(String url, JSONObject jsonObject) {
        RequestBody requestBody = RequestBody.create(JSON, jsonObject.toString());
        Request.Builder builder = new Request.Builder();
//        Map<String, String> map = ConditionsUtils.getHeader();
...
//                    String dirPath = SDCardUtils.getSDCardPath() + "/NokeLockCach";
//                    File file = new File(dirPath);
//                    if (!file.exists()){
//                        file.mkdirs();
//                    }
//                    File bitmapPath = new File(file, url.substring(url.lastIndexOf("/") + 1));
//                    boolean save = ImageUtils.save(bm, bitmapPath, Bitmap.CompressFormat.JPEG);
//                    if (save){
//                        UtilSharedPreference.saveString(view.getContext(),"url",url);
//                        UtilSharedPreference.saveString(view.getContext(),"head",bitmapPath.getAbsolutePath());
//                    }
//                    mDelivery.post(new Runnable() {
//                        @Override
//                        public void run() {
//                            if (bm != null) {
//                                view.setImageBitmap(ImageUtils.toRoundBitmap(bm));
//                            }
//
//                        }
//                    });
...
//            @Override
//            public void run() {
//                view.setImageResource(R.mipmap.use_icon);
//            }
//        });
//    }
    ...
    public static void getAsynWithHeader(String url, StringCallback callback) {
        getInstance()._getAsynWithHeader(url, callback);
    }
    ...
    public static void postJson(String url, StringCallback callback, String jsonObject) {
        getInstance()._postJson(url, callback, jsonObject);
    }

    public static void postJsonAndHeader(String url, StringCallback callback, JSONObject jsonObject) {
        getInstance()._postJsonAndHeader(url, callback, jsonObject);
    }
...
//        params = validateParam(params);
        ...
        Log.d(TAG, "buildMultipartFormRequest: "+jsonObject.toString());
